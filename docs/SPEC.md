...
λ           = funding / grossClaims ≤ 1           (si funding = 0, no hay claims)
...
- `grossClaims` (keeper, verificable desde eventos) solo determina el reparto λ. **El contrato exige `grossClaims ≥ funding` en la publicación** (siempre satisfacible con datos honestos, por superaditividad `Σ max(0,·) ≥ max(0, Σ·)`): garantiza λ ≤ 1 y elimina el varado total con λ = 0. Alcance honesto del residual (§15): un keeper que sobre-declare infra-paga a todos (el residuo rueda al siguiente settlement, nunca al manager); uno que sub-declare sobre-adjudica la tranche y el `Σ claims ≤ funding` del contrato deja a los últimos en cobrar esperando al rollover. En ningún caso aumenta la salida del stake ni la dirige a una cuenta cómplice (los `loss_lp` son estado on-chain).
...
1. Validar precondiciones y marca (§9). 2. Computar `Pe`; first-loss: funding a la `CompensationReserve`, publicación de `grossClaims` (con `grossClaims ≥ funding` exigido por contrato) y `NI −= funding` (§6). **No hay reset de `NI`** — el capital asegurado es de por vida (v0.6). 3. Perf fee (§7.2); el `FeeSplitter` está **excluido de la contabilidad NI por completo** (sus mints no suman, sus redenciones no restan, no genera claims — si restara, cada cobro de fees reduciría el funding de los LPs; `NI ≡ Σ NI_lp` se define sobre cuentas de LP). 4. Ejecutar reducción de stake pendiente si cumple condiciones.
...
21. **Keeper publica `grossClaims` incorrecto** → el funding es exacto y on-chain (`min(stakeDisponible, max(0, NI − Pe × totalShares))`) y el contrato exige `grossClaims ≥ funding` en la publicación (λ ≤ 1 por construcción, sin funding varado): un `grossClaims` falso solo redistribuye λ entre claimants, jamás aumenta la salida del stake ni la quema; escalar recomputable por cualquiera desde eventos (§15).
