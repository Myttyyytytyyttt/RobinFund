  ...
  const act = async (agent: AgentDashboardRecord, action: 'pause' | 'rotate' | 'resume') => {
    ...
    try {
      if (action === 'pause') {
        ...
        if (!alreadyActive) {
          const currentSigner = beforeActivation.agent?.signer
          if (!currentSigner || !isAddress(currentSigner)) {
            throw new Error('Agent sync did not return the current onchain signer.')
          }
          ...
          await activateWorldBacking(wallet as BrowserWallet, {
            agentId,
            signer: getAddress(currentSigner),
          }, token)
