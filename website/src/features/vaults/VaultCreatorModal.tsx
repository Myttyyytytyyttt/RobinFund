...
function Choice({ active, title, text, onClick }: { active: boolean; title: string; text: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`cursor-pointer rounded-2xl border p-4 text-left transition-colors ${active ? 'border-emerald-200/40 bg-emerald-200/10' : 'border-white/10 bg-black/20 hover:bg-white/5'}`}>
...
export function VaultCreatorModal({ open, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    const escape = (event: KeyboardEvent) => event.key === 'Escape' && !busy && onClose()
  ...
  const finishAgent = async (deployment: AgentVaultDeployment, afterFinalize?: () => Promise<void>) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const job = await submitAgentVaultJob(input, token)
      setJobId(job.id)
      ...
      if (!registration.registered) {
        ...
        await submitWorldRegistrationProof(input.agentId, proof, token)
        registration = await waitForWorldRegistration(input.agentId, token)
      }
      if (!registration.registered) throw new Error('World AgentBook registration was not confirmed.')
      setWorldConnectorUri(null)
      setStatus('backing')
      await activateWorldBacking(signer as BrowserWallet, input, token)
      await syncAgentProfile(input.agentId, token)
      setStatus('deploying')
      const deployed = await waitForAgentVaultDeployment(job.id, input, token)
      setAgentDeployment(deployed)
  ...
  const steps = managerType === 'human'
    ...
    : [['Agent identity', status === 'provisioning'], ['Register', status === 'registering'], ['Nuvem World', status === 'world_nuvem'], ['AgentBook', status === 'world_agentbook'], ['World backing', status === 'backing'], ['Deploy', status === 'deploying'], ['Bind + stake', status === 'binding'], ['Active', status === 'success']]

  return (
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-[#020709]/80 px-3 py-4 backdrop-blur-xl sm:px-6 sm:py-8" role="dialog" aria-modal="true" aria-labelledby="vault-creator-title" onMouseDown={(event) => event.currentTarget === event.target && !busy && onClose()}>
      <div className="relative mx-auto w-full max-w-5xl overflow-hidden rounded-[28px] border border-white/15 bg-[#071012]/95 text-white shadow-[0_28px_100px_rgba(0,0,0,0.65)]">
        ...
        <div className="relative z-10">
          <header className="flex items-start justify-between gap-5 border-b border-white/10 px-5 py-5 sm:px-8 sm:py-6">
            ...
            <button type="button" disabled={busy} onClick={onClose} aria-label="Close creator" className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-white/15 bg-black/20 text-xl text-white/60 hover:bg-white/10 disabled:opacity-30">×</button>
          </header>
          ...
          <div className="grid md:grid-cols-[1fr_285px]">
            <form className="space-y-5 border-b border-white/10 p-5 sm:p-8 md:border-b-0 md:border-r" onSubmit={(event) => { event.preventDefault(); void submit() }}>
              <div className="grid grid-cols-2 gap-3">
                <Choice active={managerType === 'human'} title="Human manager" text="Your connected wallet controls approved Fund trades." onClick={() => { setManagerType('human'); setError(null) }} />
              ...
              {managerType === 'ai' && (
                <div className="space-y-4 rounded-2xl border border-emerald-200/15 bg-emerald-200/[0.045] p-4 sm:p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3"><Choice active={runtimeKind === 'external'} title="External agent" text="Run any model from your PC, VPS or cloud via the SDK." onClick={() => { setRuntimeKind('external'); setAgentId(null); setManagedIdentity(null) }} /><Choice active={runtimeKind === 'nuvem_reference'} title="Nuvem reference" text="One-click identity and the transparent Nuvem runtime." onClick={() => { setRuntimeKind('nuvem_reference'); setAgentId(null) }} /></div>
              ...
              <button type="submit" disabled={!authenticated || walletMismatch || busy || status === 'success' || status === 'queued'} className="w-full cursor-pointer rounded-xl bg-white px-5 py-3.5 text-sm font-semibold text-gray-950 transition-transform hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-35">
                {!authenticated ? 'Connect wallet from the navbar' : walletMismatch ? 'Reconnect the active wallet first' : status === 'provisioning' ? 'Creating isolated agent identity…' : status === 'preparing' ? 'Preparing sponsor wallet…' : status === 'registering' ? 'Register agent in your wallet…' : status === 'world_nuvem' ? 'Waiting for Nuvem World ID…' : status === 'world_agentbook' ? 'Waiting for AgentBook…' : status === 'backing' ? 'Activating World backing in your wallet…' : status === 'deploying' ? 'Deploying contracts…' : status === 'binding' ? 'Authorize, bind and stake…' : status === 'approving' ? 'Lock protection in your wallet…' : status === 'queued' ? 'Deployment queued' : status === 'success' ? 'Vault is live' : agentDeployment || humanDeployment ? 'Resume launch' : managerType === 'ai' ? 'Create AI vault' : 'Create human vault'}
              </button>
            </form>
